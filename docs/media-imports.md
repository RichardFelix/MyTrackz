...
| status         | **Yes**   | Completed                                      | One of: `Completed`, `In progress`, `Planning`, `Paused`, `Dropped`. Capitalisation must match. The stored/imported value `Planning` is displayed as **Backlog** in the UI.       |
